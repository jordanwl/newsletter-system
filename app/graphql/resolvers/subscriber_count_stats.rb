        ...
        when "DAYS"
          ...
          newsletters.each do |newsletter|
            ...
            date_range.each do |date|
              ...
              unsubs = subs_within_range.where(action: "unsubscribe").count

              sub_counter = subs.values[0].to_i - unsubs.values[0].to_i

              sub_count_stats["#{newsletter.name}, id: #{newsletter.id}"][date] = sub_counter
            end
          end
        when "WEEKS"
          date_range = ((Date.today - 7.weeks)..Date.today.end_of_day)
