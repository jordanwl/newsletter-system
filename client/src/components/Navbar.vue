<template>
  <nav class="navbar navbar-expand-lg navbar-dark border-bottom bg-dark justify-content-between">
    <div class="navbar-nav">
      <router-link class="nav-item nav-link text-white" to="/">Home</router-link>
      <router-link class="nav-item nav-link text-white" to="/new">New Email</router-link>
    </div>
    <div class="navbar-nav">
      <router-link v-if="!$store.state.isLoggedIn" class="nav-item nav-link" to="/signin" >Sign-in</router-link>
      <a v-else class="nav-item nav-link text-white" href="#" @click="logout">Logout</a>
